#!/usr/bin/env bash
# Build LULESH from the repository root using the Intel environment; inputs
# are the source tree and optional BUILD_JOBS/BUILD_TYPE overrides; output is
# the Intel-specific build directory and PBS-captured build metadata.
set -euo pipefail

REPO_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
BUILD_NAME="Intel-CrayMPICH8"
...
record
record '== toolchain =='
for tool in CC cmake mpirun mpiexec; do record "${tool}: $(command -v "${tool}")"; done
...
record_cmd cmake \
  ...
  -DWITH_MPI=On -DWITH_OPENMP=On -DWITH_SILO=Off \
  -DCMAKE_INSTALL_PREFIX="${INSTALL_PREFIX}" "${SOURCE_DIR}"
record '== cmake build =='
