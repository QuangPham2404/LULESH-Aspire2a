#!/usr/bin/env bash
# Build LULESH from the repository root using the GNU environment; inputs are
# the source tree and optional BUILD_JOBS/BUILD_TYPE overrides; output is the
# GNU-specific build directory and PBS-captured build metadata.
set -euo pipefail

REPO_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
BUILD_NAME="GNU-CrayMPICH8"
